    ...
    try {
      ...
      if (url.pathname === "/api/organizations" && request.method === "GET") {
        ...
        const values = await Promise.all(state.accounts.map(async (account) => {
          ...
          if (repo) { const statuses = workflowStatuses(catalog, await repositorySnapshot(client, account.login, ".agents")); counts = { installed: statuses.filter((s) => s.state === "installed").length, outdated: statuses.filter((s) => s.state === "outdated").length, overridden: statuses.filter((s) => s.state === "overridden").length }; }
      ...
      if (orgWorkflows && request.method === "GET") {
        ...
        let repo; try { repo = await repository(client, login); } catch (error) { const status = (error as { status?: number }).status; if (status === 404) return json({ login, repository: null, repositoryUrl: `https://github.com/${encodeURIComponent(login)}/.agents`, manageable, workflows: publicCatalog.map((item) => ({ ...item, state: "add", action: manageable ? "add" : "none" })) }); throw error; }
        if (repo.private && !manageable) return json({ error: "Repository is private or unavailable" }, 404);
        const statuses = workflowStatuses(catalog, await repositorySnapshot(client, login, ".agents"));
        return json({ login, manageable, private: repo.private, repositoryUrl: `https://github.com/${encodeURIComponent(login)}/.agents`, workflows: publicCatalog.map((item) => ({ ...item, ...statuses.find((status) => status.id === item.id), action: manageable ? statuses.find((status) => status.id === item.id)?.action : "none" })) }, manageable ? 200 : 200);
      }
      const orgPlans = url.pathname.match(/^\/api\/orgs\/([^/]+)\/plans$/);
      if (orgPlans && request.method === "POST") { const login = decodeURIComponent(orgPlans[1]); const state = await scoped(env, request); if (!state.accounts.some((account) => account.login === login)) return json({ error: "Account is not manageable" }, 403); const body = await request.json<{ workflow?: string }>(); const client = await github(env, request); const plan = await buildPlan(client, { repository: `${login}/.agents`, managedRoot: "", catalog, workflow: body.workflow }); return json({ plan, token: await signPlan(plan, env.AGENTS_PLAN_SIGNING_KEY) }); }
      ...
      if (url.pathname === "/api/agents/plans" && request.method === "POST") { const client = await github(env, request); const body = await request.json<{ repository: string; managedRoot: string; workflow: string }>(); const plan = await buildPlan(client, { repository: body.repository, managedRoot: body.managedRoot, workflow: body.workflow, catalog }); return json({ plan, token: await signPlan(plan, env.AGENTS_PLAN_SIGNING_KEY) }); }
