...
describe("dashboard client", () => {
  ...
  it("routes an account without a repository to onboarding and walks the steps", async () => {
    ...
    const fetcher = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (path === "/api/accounts/acme/playground") { playground = { repository: { fullName: "acme/outfitter-playground", url: "https://github.com/acme/outfitter-playground", defaultBranch: "main", created: true }, issue: { number: 1, url: "https://github.com/acme/outfitter-playground/issues/1", title: "Add a describeConfig helper", created: true } }; return Response.json(playground); }
